...
fn main() {
    let matches = App::new("nixf-diagnose")
        ...
        .arg(Arg::with_name("nixf-tidy-path")
            .long("nixf-tidy-path")
            .value_name("PATH")
        ...
        .arg(Arg::with_name("input")
            ...
            .required(true)
            .takes_value(true))
        .arg(Arg::with_name("variable-lookup")
    ...
    let nixf_tidy_path = matches.value_of("nixf-tidy-path")
        .map(|p| p.to_string())
    ...
    let variable_lookup = matches.value_of("variable-lookup").unwrap().parse::<bool>().unwrap_or(true);
    ...
    File::open(input_file).expect("Failed to open input file")
        .read_to_string(&mut input).expect("Failed to read input file");

    let mut child = cmd.spawn().expect("Failed to execute nixf-tidy");
    child.stdin.as_mut().unwrap().write_all(input.as_bytes()).expect("Failed to write to stdin");

    let output = child.wait_with_output().expect("Failed to read nixf-tidy output");
    ...
    if let Some(diags) = diagnostics.as_array() {
        for diag in diags {
            if let (Some(message), Some(spans), Some(severity), Some(args), Some(notes)) = (diag.get("message"), diag.get("range"), diag.get("severity"), diag.get("args"), diag.get("notes")) {
                ...
                if let (Some(start), Some(end)) = (
                    spans.get("lCur").and_then(|s| s.get("offset").and_then(|o| o.as_u64())),
                    spans.get("rCur").and_then(|e| e.get("offset").and_then(|o| o.as_u64())),
                ) {
                    ...
                    if let Some(notes_array) = notes.as_array() {
                        for note in notes_array {
                            if let (Some(note_message), Some(note_args), Some(note_spans)) = (note.get("message"), note.get("args"), note.get("range")) {
                                let mut formatted_note_message = note_message.as_str().unwrap_or("Unknown note").to_string();
                                if let Some(note_args_array) = note_args.as_array() {
                                    for arg in note_args_array {
                                        if let Some(arg_str) = arg.as_str() {
                                            formatted_note_message = formatted_note_message.replacen("{}", arg_str, 1);
                                        }
                                    }
                                }

                                if let (Some(note_start), Some(note_end)) = (
                                    note_spans.get("lCur").and_then(|s| s.get("offset").and_then(|o| o.as_u64())),
                                    note_spans.get("rCur").and_then(|e| e.get("offset").and_then(|o| o.as_u64())),
                                ) {
                                    report = report.with_label(
                                        Label::new((input_file, note_start as usize..note_end as usize))
